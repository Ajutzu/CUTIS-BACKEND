...
export const moveImageToFolder = async (imageUrl, targetFolder, originalFilename) => {
...
export const findConditionByClassification = async (classification) => {
  try {
    const condition = await Condition.findOne({ name: { $regex: new RegExp(classification, 'i') } });
    return condition;
  } catch (error) {
